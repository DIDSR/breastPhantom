...
// based on "A computational model to generate simulated three-dimensional
...
int main(int argc, char* argv[]){
	...
	double sigma2 = rNormGen->GetScaledValue(vm["mass.meanSigma2"].as<double>(),
	...
	for(int l=0; l<=lMax; l++){
		...
		for(int m=0; m<=l; m++){
			if(m == 0){
				// debug for cluster
				//slm[l][m].real(rNormGen->GetValue()*sqrt(2.0));
				//rNormGen->Next();
				//slm[l][m].imag(0.0);
				...
				// debug for cluster
				//slm[l][m].real(rNormGen->GetValue());
				//rNormGen->Next();
				//slm[l][m].imag(rNormGen->GetValue());
				//rNormGen->Next();
	...
	for(int i=0; i<numLF; i++){
		LF[i].length = rNormGen->GetScaledValue(vm["mass.meanLFLen"].as<double>(),
	...
	//int massDim = static_cast<int>(ceil(1.1*2*rmax/imgRes));
	...
	writer->SetFileName("testMass.vti");
#if VTK_MAJOR_VERSION <= 5
	...
	for(int i=massExtent[0]; i<=massExtent[1]; i++){
		for(int j=massExtent[2]; j<=massExtent[3]; j++){
			double total = 0.0;
			unsigned char* p = static_cast<unsigned char*>(mip->GetScalarPointer(i,j,0));
			for(int k=massExtent[4]; k<=massExtent[5]; k++){
