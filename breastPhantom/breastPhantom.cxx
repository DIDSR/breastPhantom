...
int main(int argc, char* argv[]){
	...
	* **********************/
	...
	// timing
	time(&currentTime);
	//cout << "\nTime interval 11 " << difftime(currentTime,previousTime) << "\n";
	previousTime = currentTime; 
	...
	for(int i=backPlaneInd; i<dim[0]; i++){
		double coords[3];
			...
			for(int k=0; k<dim[2]; k++){
				...
				if(p[0] == innerVal){
					...
					for(int n=0; n<=numBreastCompartments; n++){
						...
						dist += boundaryDev*dist*boundary[n].getNoise(localCoords.GetData());
	...
	// fat volume need to count voxel by voxel to capture backplane
	unsigned int fatVoxels = 0;
	...
	cout << "done.\n";
	cout << "Finished compartment generation, glandular fraction = " << glandVol/(glandVol+fatVol) << "\n";
	...
	// check boundary voxels and remove fat, muscle and near-nipple voxels
	...
	for(int i=0; i<nBoundary; i++){
		...
		if(p[0] == tissue.fat || p[0] == tissue.muscle || vtkMath::Distance2BetweenPoints(loc, nipplePos) < areolaRad*areolaRad*2){
	...
	for(int i=0; i<boundaryList->GetNumberOfIds(); i++){
		...
		double loc[3];
		breast->GetPoint(myId, loc);
		startPts->InsertNextPoint(loc);
	}
	
	// create a vtkPolyData
	vtkSmartPointer<vtkPolyData> startSet =
		vtkSmartPointer<vtkPolyData>::New();
	startSet->SetPoints(startPts);

	// add verticies
	vtkSmartPointer<vtkVertexGlyphFilter> vertexGlyphFilter4 =
		vtkSmartPointer<vtkVertexGlyphFilter>::New();
	
	vertexGlyphFilter4->AddInputData(startSet);
	vertexGlyphFilter4->Update();
	
	// output
	vtkSmartPointer<vtkXMLPolyDataWriter> writerD2 =
		vtkSmartPointer<vtkXMLPolyDataWriter>::New();
	writerD2->SetFileName("startPoints.vtp");
#if VTK_MAJOR_VERSION <= 5
	writerD2->SetInput(vertexGlyphFilter4->GetOutput());
#else
	writerD2->SetInputConnection(vertexGlyphFilter4->GetOutputPort());
#endif
	writerD2->Write();
	...
	***********************/
	...
	// timing
	time(&currentTime);
	cout << "Time interval 12: " << difftime(currentTime,previousTime) << " seconds.\n";
	previousTime = currentTime; 
	...
	TDLUloc = new vtkSmartPointer<vtkPoints>[numBreastCompartments];
	
	vtkSmartPointer<vtkDoubleArray> *TDLUattr;
	TDLUattr = new vtkSmartPointer<vtkDoubleArray>[numBreastCompartments];
	...
	#pragma omp parallel
	{
		...
		for(int i=0; i<numBreastCompartments; i++){
			...
			#pragma omp critical
			{
				cout << "Breast compartment " << i << ": " << glandCompartments[i].voxelCount*imgRes*imgRes*imgRes/1000.0 <<
					" cc (" << glandCompartments[i].voxelCount << " voxels).\n";
			}
			...
			for(int j=0; j<3; j++){
				sdir[j] = glandCompartments[i].pos[j] - nipplePos[j];
			}
			// normalize
			...
			while(!inCompartment && dist < snorm){
				...
				if(p[0] == compartmentVal[glandCompartments[i].compId]){
					// inside compartment
					inCompartment = true;
				} else {
			...
			generate_duct(breast, vm, TDLUloc[i], TDLUattr[i], compartmentVal[glandCompartments[i].compId], 
				glandCompartments[i].boundBox, &tissue, currentPos, sdir, seed);
		}
	}
	...
	// timing
	time(&currentTime);
	cout << "Time interval 13: " << difftime(currentTime,previousTime) << " seconds.\n";
	previousTime = currentTime;
	...
	double currentFatFrac = fatVol/(glandVol+fatVol);
	...
	double minSkinAxialRatio = 0.13; // min and max lobule axial ratio
	double maxSkinAxialRatio = 0.3;
	double skinLobuleDeflectMax = 0.15; // deflection fraction of pi
	double skinPerturbMax = 0.15;
	double skinLigThick = 0.35; // ligament thickness (mm);

	double targetSkinFatFrac;
	if(currentFatFrac >= targetFatFrac){
		targetSkinFatFrac = currentFatFrac;
	} else {
		targetSkinFatFrac = currentFatFrac + (targetFatFrac-currentFatFrac)*0.333;
	}
	
	int maxSkinLobules = 3000;
	...
	while(numSkinLobules < maxSkinLobules && numGlandBoundary > 0 && currentFatFrac < targetSkinFatFrac){
		 
		// pick a random voxel from the boundary list
		int seedVox[3];
		int randVal = (int)(floor(rgen->GetRangeValue(0, numGlandBoundary)));
		rgen->Next();
		...
		double searchRad = A*(1+skinPerturbMax);
		double originalA = A;
			...
			for(int j=segSpace[2]; j<= segSpace[3]; j++){
				for(int k=segSpace[4]; k<= segSpace[5]; k++){
					...
					if(*p == tissue.TDLU || *p == tissue.duct){
						...
						double perturbVal = skinPerturbMax*perturb.getNoise(spherePos);
		...
		for(int i=0; i<numGlandBoundary; i++){
			...
			if(p[0] == tissue.fat || p[0] == tissue.cooper){
				// set to remove from list
				boundaryDel->InsertNextId(myId);
			}
		}
		...
		// debug
		if(!(numSkinLobules % 20)){
			cout << "skin lobule " << numSkinLobules << ", fat frac = " << currentFatFrac 
				<< ", target ff = " << targetSkinFatFrac << ", numGlandBoundary = " << numGlandBoundary << "\n";
		}
	}
		...
		for(int i=segSpace[0]; i<= segSpace[1]; i++){
			for(int j=segSpace[2]; j<= segSpace[3]; j++){
				for(int k=segSpace[4]; k<= segSpace[5]; k++){
					...
					if(*p == tissue.gland || *p == tissue.TDLU){
						...
						// inside lobule?
						if(r <= A*(f + perturbVal) - innerLigThick){
							// gland to fat
							*p = tissue.fat;
							#pragma omp atomic
							glandVoxels -= 1;
							...
							*p = tissue.cooper;							
							...
							// in buffer?
							if(r <= A*(f + perturbVal + bufferVal)){
								// gland to buffer gland
								*p = glandBuffer;
							}
						}
					}
	...
	/*******************
	 * Cooper's Ligaments in Fat
	 * */
	
	double ligThick = 0.35;
	 
	const unsigned char ligFat = 99;
	double maxFatLigAxis = 10.0;
	double minFatLigAxis = 8.0;
	double maxFatLigAxisRatio = 0.7;
	double minFatLigAxisRatio = 0.4;
	double fatLigPerturbMax = 0.25;
	double fatLigDeflectMax = 0.15;
	unsigned int numFatLigs = 7*(unsigned int)ceil(fatVoxels*voxelVol/(4.0/3.0*pi*pow((maxFatLigAxis+minFatLigAxis)/2.0,3.0)));
	
	int maxFatLigTry = 15000;
	int flnum = 0;
	int fltry = 0;
	while(flnum < numFatLigs && fltry < maxFatLigTry){  
	
	 
	//for(int lnum=0; lnum<numFatLigs; lnum++){
		...
		// found a seed, pick a size
		double axisLen[3];
		axisLen[0] = rgen->GetRangeValue(minFatLigAxis,maxFatLigAxis);
		rgen->Next();
		for(int i=1; i<3; i++){
			axisLen[i] = axisLen[0]*rgen->GetRangeValue(minFatLigAxisRatio,maxFatLigAxisRatio);
			rgen->Next();
		}
		...
		dtheta = rgen->GetRangeValue(0,2*pi);
		rgen->Next();
		dphi = rgen->GetRangeValue(0,fatLigDeflectMax);
		rgen->Next();
		double dr = tan(pi*dphi);
		...
		int32_t perturbSeed = (int32_t)(ceil(rgen->GetRangeValue(-1073741824, 1073741824)));
		rgen->Next();
		//perlinNoise perturb(vm, perturbSeed, A*0.002, 1.25, 0.5, 6);
		perlinNoise perturb(vm, perturbSeed, A*0.004, 1.1, 0.5, 6);
		
		double searchRad = A*(1+fatLigPerturbMax)+ligThick;
	...
	cout << "Created " << flnum << " of " << numFatLigs <<" fat ligaments\n";
	...
	unsigned int numGlandLigs = 3.0*(unsigned int)ceil(glandVoxels*voxelVol/(4.0/3.0*pi*pow((maxGlandLigAxis+minGlandLigAxis)/2.0,3.0)));
	...
	double vesselEdgeSep = 12;  // distance from edge of breast of vessel entry point (mm)
	...
	for(int i=0; i<4; i++){
		...
		if(i == 0){
			generate_artery(breast, vm, internalExtentVox, &tissue,
				arteryStartPosList[i], arteryStartDirList[i], arterySeed, true);
		} else {
			generate_artery(breast, vm, internalExtentVox, &tissue,
				arteryStartPosList[i], arteryStartDirList[i], arterySeed, false);
		}
	}
	...
	for(int i=0; i<5; i++){
		...
		if(i == 0){
			generate_vein(breast, vm, internalExtentVox, &tissue,
				veinStartPosList[i], veinStartDirList[i], veinSeed, true);
		} else {
			generate_vein(breast, vm, internalExtentVox, &tissue,
				veinStartPosList[i], veinStartDirList[i], veinSeed, false);
		}
	}
	...
	unsigned int* lig2DProj = (unsigned int*)calloc(dim[0]*dim[2],sizeof(unsigned int));
	unsigned int* duct2DProj = (unsigned int*)calloc(dim[0]*dim[2],sizeof(unsigned int));
	unsigned int* gland2DProj = (unsigned int*)calloc(dim[0]*dim[2],sizeof(unsigned int));
	unsigned int* TDLU2DProj = (unsigned int*)calloc(dim[0]*dim[2],sizeof(unsigned int));
	unsigned int* artery2DProj = (unsigned int*)calloc(dim[0]*dim[2],sizeof(unsigned int));
	unsigned int* vein2DProj = (unsigned int*)calloc(dim[0]*dim[2],sizeof(unsigned int));
	...
	for(int i=0; i<dim[0]; i++){
		//cout << "Projecting slice " << i+1 << " of " << dim[0] << "\n";
		for(int k=0; k<dim[2]; k++){
			for(int j=0; j<dim[1]; j++){
				unsigned char* p = static_cast<unsigned char*>(breast->GetScalarPointer(i,j,k));
			...
			for(int j=startSlice; j<startSlice+sliceThickInd; j++){
				unsigned char* p = static_cast<unsigned char*>(breast->GetScalarPointer(i,j,k));
				if(p[0] == tissue.cooper){
					lig2DProj[i*dim[2]+k] = lig2DProj[i*dim[2]+k] + 1;
				} else if(p[0] == tissue.fat){
					fat2DProj[i*dim[2]+k] = fat2DProj[i*dim[2]+k] + 1;
				} else if(p[0] == tissue.skin){
					skin2DProj[i*dim[2]+k] = skin2DProj[i*dim[2]+k] + 1;
				} else if(p[0] == tissue.duct){
					duct2DProj[i*dim[2]+k] = duct2DProj[i*dim[2]+k] + 1;
				} else if(p[0] == tissue.TDLU){
					TDLU2DProj[i*dim[2]+k] = TDLU2DProj[i*dim[2]+k] + 1;
				} else if(p[0] == tissue.gland){
					gland2DProj[i*dim[2]+k] = gland2DProj[i*dim[2]+k] + 1;
				} else if(p[0] == tissue.artery){
					artery2DProj[i*dim[2]+k] = artery2DProj[i*dim[2]+k] + 1;
				} else if(p[0] == tissue.vein){
					vein2DProj[i*dim[2]+k] = vein2DProj[i*dim[2]+k] + 1;
				} else if(p[0] == tissue.nipple){
	...
	fclose(projFile);
	projFile = fopen("lig2DProj.dat","wb");
	fwrite(lig2DProj, dim[0]*dim[2], sizeof(unsigned int), projFile);
	fclose(projFile);
	projFile = fopen("duct2DProj.dat","wb");
	fwrite(duct2DProj, dim[0]*dim[2], sizeof(unsigned int), projFile);
	fclose(projFile);
	projFile = fopen("gland2DProj.dat","wb");
	fwrite(gland2DProj, dim[0]*dim[2], sizeof(unsigned int), projFile);
	fclose(projFile);
	projFile = fopen("TDLU2DProj.dat","wb");
	fwrite(TDLU2DProj, dim[0]*dim[2], sizeof(unsigned int), projFile);
	fclose(projFile);
	projFile = fopen("artery2DProj.dat","wb");
	fwrite(artery2DProj, dim[0]*dim[2], sizeof(unsigned int), projFile);
	fclose(projFile);
	projFile = fopen("vein2DProj.dat","wb");
	fwrite(vein2DProj, dim[0]*dim[2], sizeof(unsigned int), projFile);
	fclose(projFile);
