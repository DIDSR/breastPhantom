...
veinBr::veinBr(double* spos, double* sdir, double r, veinTree *owner){
	...
	if (nChild == 0){
		...
		firstChild = new veinBr(this,level,gen+1,radii[0]);
		lastChild = firstChild;
		for(int i=1; i < nChild; i++){
			// set up other children with higher level than current
			lastChild->nextBranch = new veinBr(this,lastChild,level+1,gen+1,radii[i]);
...
veinBr::veinBr(veinBr* par, unsigned int lev, unsigned int g, double r){
...
veinBr::veinBr(veinBr* par, veinBr* par2, unsigned int lev, unsigned int g, double r){
...
void veinBr::setRadii(double* radii){
...
void veinBr::setDir(double* sdir){
