...
ductTree::ductTree(po::variables_map o, ductTreeInit *init):
	randGen(init->seed),
	u01(randGen),
	lengthDist(o["ductSeg.lengthBetaA"].as<double>(),o["ductSeg.lengthBetaB"].as<double>()),
...
ductBr::ductBr(double* spos, double* sdir, double r, ductTree *owner){
	...
	if (nChild == 0){
		...
		firstChild = new ductBr(this,level,gen+1,radii[0]);
...
ductBr::ductBr(ductBr* par, unsigned int lev, unsigned int g, double r){
...
ductBr::ductBr(ductBr* par, ductBr* par2, unsigned int lev, unsigned int g, double r){
...
double ductBr::setLength(void){
	...
	double minLen0 = myTree->opt["ductBr.minLen0"].as<double>();
	double maxLen0 = myTree->opt["ductBr.maxLen0"].as<double>();
	double minLen1 = myTree->opt["ductBr.minLen1"].as<double>();
	double maxLen1 = myTree->opt["ductBr.maxLen1"].as<double>();
	double minLen2 = myTree->opt["ductBr.minLen2"].as<double>();
	double maxLen2 = myTree->opt["ductBr.maxLen2"].as<double>();
	double minLenDefault = myTree->opt["ductBr.minLenDefault"].as<double>();
	double maxLenDefault = myTree->opt["ductBr.maxLenDefault"].as<double>();
...
void ductBr::setRadii(double* radii){
	...
	double radFrac0 = myTree->opt["ductBr.radFrac0"].as<double>();
...
void ductBr::setDir(double* sdir){
	...
	double angleMin = pi*(myTree->opt["ductBr.minAngle"].as<double>());
	double angleMax = pi*(myTree->opt["ductBr.maxAngle"].as<double>());
